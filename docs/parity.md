...
| `Package (JPRM) / Generate SBOM` | here | The component inventory, written in the same run as the package and attached to it, from #101. One per line rather than one per run: the tool aggregates every target framework when it is told none, and the two lines reference different server packages, so an aggregate would describe neither archive. It is read off the locked restore rather than off a fetch of its own, so it describes the graph `packages.lock.json` holds. Nothing publishes it yet; #118 is the release half. Not required by the ruleset; #105. |
