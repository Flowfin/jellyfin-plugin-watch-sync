...
| --- | --- | --- |
| `build` | here | `build`, from `.github/workflows/build.yaml`, from #90. It was `call / build`, a call into the shared plugin workflow, until that change: the call packaged rather than built, and the packager takes one framework, so the required check compiled one of the two lines this plugin ships. This one builds the solution for both, with the analyzer settings and the warnings-as-errors the project itself declares rather than whatever the shared tree passed, and a probe step proves a warning is refused rather than leaving that to be assumed. The condition is met for the plugin project and not for the test project, which declares no `TreatWarningsAsErrors` and carries analyzer warnings; the count and the command are on #90 and that half is what is left of it. Whether the ruleset names this context is #105, read with the command at the top of this file. |
...
| `Package (JPRM) / Build package` | here | `package / Package and inventory`, from #101. Packaging runs on every pull request, at the packager commit the release route uses, which `PackagingGateTests` holds the two files to. It builds every line the manifests declare, one archive each: the packager stamps the archive with the top level ABI of the manifest standing at `build.yaml` rather than with the framework it was told to build, so the second call is preceded by the same swap the release route makes for a tagged line, and `PackagingGateTests` holds every call to the framework of the manifest in front of it. #117 is the release half. Not required by the ruleset; #105. |
| `Package (JPRM) / Generate SBOM` | here | The component inventory, written in the same run as the package and attached to it, from #101. One per archive rather than one per run, each under its own line's directory. The two hold the same components: the tool is given the framework and does not narrow the closure while the restore it reads carries every declared target, which was measured rather than assumed and is recorded at the step that writes them. It is read off the locked restore rather than off a fetch of its own, so it describes the graph `packages.lock.json` holds, which is more than either archive carries and never less. Nothing publishes it yet; #118 is the release half. Not required by the ruleset; #105. |
...
| --- | --- | --- |
| `test` | here | A suite check. The other board runs `dotnet.yml` and does not require one; this board has required a test check from #7 onward, because the failure mode here is silent data loss and a suite that can be skipped is not a control. It was `call / test`, a call into the shared plugin workflow, until #90 moved the run into `.github/workflows/test.yaml`, where the arguments are decided here rather than in a tree this repository pins but does not hold. Whether the ruleset names this context is #105, read with the command at the top of this file. |
...
| `changelog.yaml` | kept, #176 | A workflow this repository carries from the plugin template rather than by a decision of its own. It is answered on #176 and stays, skipped, until #116 decides what the release route reads its change list from, because removing it first would settle that question with a deletion. THE CONDITION THIS ROW WAITS ON IS MET AND THE ROW'S CONCLUSION IS NOT THIS CHANGE'S TO MOVE. #116 answered it: the release route reads the fragments under `changelog.d/`, assembled by `.github/assemble-release-notes.py` in the publish route's gate job, and `docs/changelog.md` carries the argument. What is left is whether this workflow is then removed, which is #176's question and is not decided here. It is skipped on every push, because it passes the name this repository carried before it moved and the shared job is guarded on that name, which is why `call / update_release_draft` appears in the mainline list above with the conclusion `skipped`; #165 says why that name is deliberately not repaired, and being skipped is the whole of what makes keeping it safe. THIS ROW ONCE NAMED TWO WORKFLOWS. `sync-labels.yaml` is the row below, and it is removed rather than kept. |
| `sync-labels.yaml` | removed, #350 | THIS ROW SAID `kept, #176` AND THE READING IT OWED HAS BEEN TAKEN. What it said was that the behaviour of the shared job against a label its set does not name was unmeasured here, that the only reading answering it was a live run against this board, and that the next first-of-month run was the observation rather than the behaviour being known. That run happened, and it deleted this board's own labels. Thirteen names came off seventy-four issues in one hundred and forty-five removals, counted below this table, and nothing was written to say why. The pin bought less than the comment above it claimed: it fixed which steps ran and fixed nothing about which labels this board was allowed to have, because the step is `EndBug/label-sync` with `delete-other-labels: true` and its list is fetched over plain https from another project's default branch at the moment the job runs. There is no setting that keeps a name, so the file is removed rather than narrowed. The decision is taken fleet-wide on jellyfin-plugin-share-links #338 and executed here on #350: the vocabulary of these boards is declared by the fleet and enforced by its own gate, and the standard names that list gave us stay as they are and need no sync to keep them. Template parity with the plugin template is one workflow narrower, and that is the decision rather than a cost this row leaves unsaid. |
...
On the `sync-labels.yaml` row, the three numbers are the run's own removals rather
than a description of them. The bot is the actor of every one and it has run once:

    gh api "repos/Flowfin/jellyfin-plugin-watch-sync/issues/events?per_page=100" --paginate --jq '.[] | select(.event=="unlabeled") | select(.actor.login=="github-actions[bot]") | .label.name' | sort -u | wc -l
    13
    gh api "repos/Flowfin/jellyfin-plugin-watch-sync/issues/events?per_page=100" --paginate --jq '.[] | select(.event=="unlabeled") | select(.actor.login=="github-actions[bot]") | .issue.number' | sort -u | wc -l
    74
    gh api "repos/Flowfin/jellyfin-plugin-watch-sync/issues/events?per_page=100" --paginate --jq '.[] | select(.event=="unlabeled") | select(.actor.login=="github-actions[bot]") | .label.name' | wc -l
    145

The file is gone from this tree, so those three numbers cannot grow. They are what
one firing cost, not a rate.
